...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
    {
        ...
        esp_mqtt_client_subscribe(mqttClient, "#", 0);
        ...
        if (!SystemState.getFlag("time_synced"))
        {
        ...
        if (event->data_len > 0 && event->topic_len > 0)
        {
            ...
            else if (topicStr == "Control/time")
            {
                ...
                */
                ...
                // The timestamp is Unix time; keep the offset for display only.
    ...
    case MQTT_EVENT_ERROR:
    {
        ...
        if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT)
        {
            if (event->error_handle->esp_transport_sock_errno == EAGAIN ||
                ...
                event->error_handle->esp_transport_sock_errno == ENOMEM)
            {
...
void MQTT_Send(String topic, String message, bool insertOwner, bool retained)
{
    ...
    if (message.length() > MQTT_MAX_CHUNK_SIZE)
    {
        ...
        while (message.length() > MQTT_MAX_CHUNK_SIZE)
        {
            ...
            esp_mqtt_client_publish(mqttClient, topic.c_str(), chunk.c_str(),
    ...
    {
        esp_mqtt_client_publish(mqttClient, topic.c_str(), message.c_str(),
...
void MQTT_change_to(bool local)
{
    ...
    xQueueSend(mqtt_control_queue, &cmd, pdMS_TO_TICKS(100));
}
