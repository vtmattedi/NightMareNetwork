...
bool ResourcesManager::hasOtherDeviceResource(const String &deviceName) const
{
    for (int i = 0; i < resourceCount_; ++i)
        if (resources_[i]->authority == ResourceAuthority::OTHER_DEVICE &&
...
void ResourcesManager::subscribeResource(const NetResource &resource, bool includeManifest)
{
    ...
    else if (resource.access == NetAccess::READ_WRITE)
    {
        subscriber_->subscribe(topicFor(resource,
            resource.kind == NetResourceKind::VALUE ? "set" : "invoke"));
    }
}
...
void ResourcesManager::subscribeAll()
{
    for (int i = 0; i < resourceCount_; ++i)
    {
        ...
        bool includeManifest = resource.authority == ResourceAuthority::OTHER_DEVICE;
        for (int j = 0; includeManifest && j < i; ++j)
            if (resources_[j]->authority == ResourceAuthority::OTHER_DEVICE &&
    ...
    for (int i = 0; i < resourceCount_; ++i)
    {
        ...
        if (resource.authority == ResourceAuthority::OTHER_DEVICE)
        {
            if (topicFilter == resource.ownerDevice.deviceName + ResourcesPath ||
                (resource.kind == NetResourceKind::VALUE &&
...
bool ResourcesManager::bindResource(NetResource *resource)
{
    ...
    const bool firstFromOwner = resource->authority == ResourceAuthority::OTHER_DEVICE &&
