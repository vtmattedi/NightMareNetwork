...
void Network::onMessage(const String& topic, const String& payload) {
    if (topic != "Control/time" && (!hasConsole() || topic != _resources.device() + "/console/in") &&
...
void Network::tick(uint32_t nowMs) {
    ...
    for (uint8_t processed = 0; processed < QUEUE_SIZE; ++processed) {
        ...
        if (message.topic == "Control/time") {
            ...
            if (_console) {
                CommandResult result = _console->run(message.payload,
                    {CommandSource::Mqtt, SettingsAccess::User});
                _transport.publish(_resources.device() + "/console/out", result.text);
            }
        }
#endif
...
#if NIGHTMARE_ENABLE_CONSOLE
    if (_console) _console->router().tick(nowMs);
#endif
}
