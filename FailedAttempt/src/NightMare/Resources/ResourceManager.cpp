...
bool ResourceManager::invoke(NetResource& action, const String& arguments) {
    ...
    if (entry->role == ResourceRole::LOCAL)
        return dispatchAction(*entry, arguments, true) == ActionStatus::OK;
...
ActionStatus ResourceManager::dispatchAction(ResourceRegistry::Entry& entry, const String& payload,
                                              bool localCall, String* localResult) {
    ...
    if (response != ActionResponse::NONE && !localCall) {
        ...
        if (first <= 0 || second <= first + 1) return ActionStatus::INVALID_ARGUMENT;
        ...
        if (!requestId || !caller.length() || caller.indexOf('/') >= 0)
    ...
    if (localCall || response == ActionResponse::NONE || !caller.length()) return status;
...
bool ResourceManager::invokeLocal(const String& id, const String& arguments) {
    String ignored;
    return invokeLocalResult(id, arguments, ignored) == ActionStatus::OK;
}

ActionStatus ResourceManager::invokeLocalResult(const String& id, const String& arguments,
                                                String& result) {
    auto* entry = _registry.find(ResourceRole::LOCAL, nullptr, id);
    if (!entry || entry->resource->kind() != NetResourceKind::ACTION) return ActionStatus::REJECTED;
    return dispatchAction(*entry, arguments, true, &result);
}

ActionStatus ResourceManager::writeLocal(const String& id, const String& requested) {
    auto* entry = _registry.find(ResourceRole::LOCAL, nullptr, id);
    if (!entry || entry->resource->kind() != NetResourceKind::VALUE ||
        entry->resource->access() != NetAccess::READ_WRITE) return ActionStatus::REJECTED;
    auto* item = binding(*entry->resource);
    return item && item->write ? item->write(item->writeContext, *entry->resource, requested)
                               : ActionStatus::REJECTED;
}
